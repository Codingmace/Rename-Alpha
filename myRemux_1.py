...
class Remux:
        ...
        def build_fmpg_cmd(self,split, subtitleList):
                ...
                '''
                customArguments = ['-preset', 'fast', '-fps_mode','vfr']
                if tune != '' and not True:
                        customArguments.append('-tune')
                        customArguments.append(tune)
                mapsArgument = ['-map','0:v?', '-map', '0:a?', '-map', '0:s?', '-map', '0:t?']
                copyArguments = ['-c:v', 'libx265','-c:a', 'libvorbis', '-c:s', 'copy', '-c:t', 'copy'] # Change this back to ssa instead of copy
#                copyArguments = ['-c:v', 'libx265','-c:a', 'aac', '-c:s', 'ssa', '-c:t', 'copy'] # Change this back to ssa instead of copy
                outputFileArguments = ['-n'] # Don't rewrite if already written
                if seriesName == '':
                ...
                ''' ./ffmpeg.exe -i 'NEE.mkv' -map 0:v? -map 0:a? -map 0:s? -map 0:t? -c:v libx265 -c:a aac -c:s ssa -c:t copy -ss 00:00:00 -to 00:00:30 -y Nee1.mkv'''
                return command
...
def deleteRewrittenOld(): # Then the database file will be called left so renaming is needed
        f = open(containDir + "database.csv", "r")
...
#runProgram()
#cleanDatabases()
#currentFolder = "./Steins Gate 1/" # Changable Variable
